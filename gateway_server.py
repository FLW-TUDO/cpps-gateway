...
def on_connect(client, userdata, flags, rc):
    log(20, "MQTT_CLIENT", "Connected with result code " + str(rc))
...
def connect_to_mqtt_broker():
    ...
    if rc == 0:
        log(
            20,
            "MQTT_CLIENT",
            "Connect to broker: {} on port: {:d} and topic: {}".format(
        ...
        log(40, "MQTT_CLIENT", "Connection failed with status code: {:d}".format(rc))
...
def mqtt_publish():
    ...
    if request.method == "POST":
        ...
        mqttc.publish(topic=topic, payload=json.dumps(payload), qos=0, retain=False)
    ...
    if request.method == "POST":
        ...
        if "addr" not in req_data:
            return make_response(
                jsonify(FAILURE='No addr defined. For example "addr": 21 (8bit)'), 400
            )
        else:
...
# TODO: Entweder await/async checken oder zweite Route definieren, die auf Antwort der ersten wartet?
...
def sadr():
    ...
    if request.method == "POST":
        ...
        if "uid" not in req_data:
            return make_response(
                jsonify(FAILURE='No uid defined. For example "uid": 21 (16bit)'), 400
            )
        else:
            uid = req_data["uid"]
        ...
        if "addr" not in req_data:
            return make_response(
                jsonify(
                    FAILURE='No destination addr defined. For example "addr": 170 (8bit)'
                ),
                400,
            )
        else:
        ...
        if "newAddr" not in req_data:
            return make_response(
                jsonify(
                    FAILURE='No new Address defined. For example "newAddr": 21 (8bit)'
                ),
                400,
            )
        else:
...
def chnl():
    ...
    if request.method == "POST":
        ...
        if "channel" not in req_data:
            return make_response(
                jsonify(FAILURE='No channel defined. For example "channel": 0'), 400
            )
        else:
...
def seti():
    ...
    if request.method == "POST":
        ...
        if "addr" not in req_data:
            return make_response(
                jsonify(FAILURE='No addr defined. For example "addr": 170'), 400
            )
        else:
            addr = int(req_data["addr"])
            if addr < 1:
                return make_response(
                    jsonify(
                        FAILURE="Address is out of range. Please select an address greater as 0!"
                    ),
                    400,
                )
        ...
        if "itemdescr" not in req_data:
            return make_response(
                jsonify(FAILURE='No itemdescr defined. For example "itemdescr": 1010'),
                400,
            )
        else:
        ...
        if "amount" not in req_data:
            return make_response(
                jsonify(FAILURE='No amount defined. For example "amout": 9'), 400
            )
        else:
...
def ping():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            packet.send()
            return make_response(
                jsonify(SUCCESS="PING send on address: {:2d}".format(addr)), 200
            )
        except Exception as err:
            return make_response(jsonify(FAILURE="{}".format(err)), 400)
...
def butn():
    ...
    if request.method == "POST":
        ...
        if "buttonId" not in req_data:
            return make_response(
                jsonify(FAILURE='No buttonId defined. For example "buttonId": 1'), 400
            )
        else:
...
def rsve():
    ...
    if request.method == "POST":
        ...
        if "addr" not in req_data:
            return make_response(
                jsonify(FAILURE='No addr defined. For example "addr": 170(8bit)'), 400
            )
        else:
            addr = int(req_data["addr"])
            if addr < 1:
        ...
        if "ordernumber" not in req_data:
            return make_response(
                jsonify(
                    FAILURE='No ordernumber defined. For example "ordernumber": 21'
                ),
                400,
            )
        else:
...
if __name__ == "__main__":
    log(
        ...
        "Prozess start with {}PID {:d}{}".format(
            shell_font_style.BOLD, os.getpid(), shell_font_style.END
        ),
    )
