...
void LPD8806::startBitbang() {
  ...
  bcm2835_gpio_write(datapin,0);
  for(uint8_t i = 8; i>0; i--) {
    bcm2835_gpio_write(clkpin,1);
    bcm2835_gpio_write(clkpin,0);
    // *clkport |=  clkpinmask;
    // *clkport &= ~clkpinmask;
  }
}
...
void LPD8806::updateLength(uint16_t n) {
  ...
  if(0 != (pixels = (uint8_t *)malloc(n + 1))) { // Alloc new data
...
void LPD8806::show(void) {
  ...
  for (i=0; i<n3; i++ ) {
    for (uint8_t bit=0x80; bit; bit >>= 1) {
      if(pixels[i] & bit)  bcm2835_gpio_write(datapin,1);
      else                 bcm2835_gpio_write(datapin,0);
      bcm2835_gpio_write(clkpin,1);
      bcm2835_gpio_write(clkpin,0);
    }
  }
